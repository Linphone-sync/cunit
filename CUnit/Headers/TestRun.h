 ...
 *
 *  1-May-2018    Added logging structure for normal text logging in Junit files
 *                (Bart Houkes)
 */
...
/* CU_LoggingRecord type definition. */
/** Data type for holding comments (linked list). */
typedef struct CU_LoggingRecord
{
  char*           strMessage; 	  /**< Test condition which failed. */
  CU_pTest        pTest;          /**< Test containing failure. */
  CU_pSuite       pSuite;         /**< Suite containing test having failure. */

  struct CU_LoggingRecord* pNext; /**< Pointer to next record in linked list. */
  struct CU_LoggingRecord* pPrev; /**< Pointer to previous record in linked list. */

} CU_LoggingRecord;
typedef CU_LoggingRecord* CU_pLoggingRecord;  /**< Pointer to CU_LoggingRecord. */
 ...
 *  or CU_run_test().
 */
CU_EXPORT CU_pLoggingRecord CU_get_comment_list(void);
/**<
 *  Retrieves the head of the linked list of comments which added during the
 *  last run (reset each run).
 */
...
CU_EXPORT void CU_error_message(const char *strFile, unsigned int uiLine, const char *strMessage, ...);
/**<
 *  Create an error logging with formatted message.
 *
 *  @param strMessage What message
 */

CU_EXPORT void CU_log(const char *strMessage, ...);
/**<
 *  Create a logging with formatted message. Added to the Junit output, not resulting in errors.
 *
 *  @param strMessage What message
 */
